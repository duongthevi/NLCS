...
export default class ApiService {
    ...
    static async searchProducts(searchValue) {
        try {
            ...
            console.log("Dữ liệu trả về:", response.data);  // Debug
            return response.data;
        } catch (error) {
    ...
    static async updateOrderitemStatus(orderId, status) {
        const response = await axios.put(`${this.BASE_URL}/order/update-item-status/${orderId}`, {}, {
            headers: this.getHeader(),
            params: {status}
        })
        return response.data;
    ...
    static async incrementCartItem(productId) {
        const response = await axios.put(`${this.BASE_URL}/cart/increment/${productId}`, {}, {
            headers: this.getHeader(),
        });
        return response.data;
    }
    ...
    //Statistics API
    static async getMonthlyStatistics(month, year) {
        const response = await axios.get(`${this.BASE_URL}/statistics/monthly`, {
            params: { month, year },
            headers: this.getHeader()
        })
        return response.data;
